...
class TransformerEncoder(nn.Module):
    """
    Transformer encoder wrapper compatible with repo tensor shapes.

    - Expects inputs shaped (T, N, E) and returns (T, N, E).
    - Accepts input_lengths (N,) to construct src_key_padding_mask where True==pad.
    """
    def __init__(
        self,
        d_model: int,
    ...
    def forward(self, inputs: torch.Tensor, input_lengths: torch.Tensor | None = None) -> torch.Tensor:
        """
        inputs: (T, N, E)
        input_lengths: (N,) (un-padded lengths in timesteps)
        returns: (T, N, E)
        """
        T, N, E = inputs.shape
